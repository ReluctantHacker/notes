...
public class basic {  // public class randomName {  ----> this also works but not recommended at all.// 
    ...
    // (String[] args) 裡面的東西其實就是一個陣列宣告. 他表示args這個變數是一個string陣列, 與C#一樣的表示法. 但與C和C++正好相反, 在C和C++中, 會使用(String args[]), 常見的例子就是int testArr[];這樣的宣告. 但是在java和C#中就是int[] testArr;剛好相反, 這只是語法不同而已.  而在main裡面的參數其實就是你執行.exe的時候要給他參數例如"./test.exe -args", 就算不是手動執行, 而是其他program執行也是一樣, 你要為給他args, 這在C, C++, C#都是一樣的, 只是在練習C的時候通常不會需要參數(正是專案不一定). 而java和C++通常寫法上會預設給main參數, 在C#中你不給其實完全也可以. 但是java中必須要, 因為java設計成執行compiled program時必定會至少吃一個string[], 即使是空的(不是NULL). 
