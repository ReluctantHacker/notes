...
> [!NOTE]
> Also noticed! When we talk about potential field doesn't change by time, we are saying it's **relative** field. Because a particle can move, and so as it's potential field. The static observer would observe that the potential generated by that particle at certain position would change over time because that particle is moving. What **time translation invariant** means is that **the physical law** won't change which means that If we are moving with that moving particle, we would see the potential field won't change by time. Actually, in two particles case, we can actually suppose of them is static because we only focus on kinetic energy and potential energy so the speed of they getting closing doesn't matter.
...
So, we have that total energy is conservative for one particle system in static gravitational field. Apparently, if kinetic energy field change by time which means potential field change by time then the total energy won't conservative anymore because for example, if a particle has 1 kinetic energy and 0 potential energy, and the infinite remote position has negative infinity potential energy. 

two identical particle attract each other by gravitation then **Kinetic energy field** which only matter with **Relative Position" between two particle.
