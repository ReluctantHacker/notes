...
# 天線接收時的RHCP和LHCP
其實就是波的偏振, 由於衛星天線會轉動, 在物理上為了防止接收信號衰減(如果使用線偏振信號無法符合旋轉需求), 所以採用圓偏振, 而圓偏振在物理維度上有兩種, 若按照旋轉的話則有順時針和逆時針, 在這裡稱作左旋和右旋. 也就是RHCP和LHCP. 而衛星天線為何不全部統一採用其中一個, 唯一的目的就是為了要讓頻率利用率加倍, 僅此而已
...
# TMI檔
衛星的主要功能是拍照(2D的), 然後太空雷達會利用反射原理提供3D圖, 然後軟體通常會自動將衛星的拍照結果盡可能不失真的貼到3D圖上, 產生的結果通常就是TMI圖檔.
